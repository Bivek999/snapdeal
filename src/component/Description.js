...
const Description = () => {
    ...
    useEffect(()=>{
        setCart(localStorage.getItem("token"))
    },[])
    function handleCart(){
        const cartselect=select;
        cartselect.numberOfItem=1
        console.log(cartselect);
        setCart((prev)=>[...prev,cartselect]);
        console.log(cart);
  ...
  return (
    <div className='descriptions'>
            ...
            <div className='describe-right'>                
                ...
                <div className='rating-style'><span>⭐{select.rating}&nbsp;&nbsp;&nbsp;Rating</span>
                </div>
                <div><h4 style={{float:"left",margin:"30px"}}>Special Price</h4></div>
                <div className='price-discount'>                    
                    <span>
                    <span >{formatAmount((select.price*80)-((select.price/100)*select.discountPercentage)*80)}</span>
                    &nbsp;&nbsp;<span style={{textDecorationLine:"line-through",color:"gray"}}>{formatAmount(select.price*80)}</span>
                    </span>
